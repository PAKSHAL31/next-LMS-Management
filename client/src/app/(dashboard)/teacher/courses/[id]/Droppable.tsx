...
export default function DroppableComponent() {
  ...
  const handleSectionDragEnd = (result: any) => {
    ...
    const startIndex = result.source.index; // the start where we start dragging the sections
    const endIndex = result.destination.index; // the end index / location where it should be dragged to.

    const updatedSections = [...sections]; // copy the sections
    //both this code will re-order the updatedSecions
    const [reorderedSection] = updatedSections.splice(startIndex, 1);
    updatedSections.splice(endIndex, 0, reorderedSection);
    dispatch(setSections(updatedSections)); // update the secitons in state redux.
  };
...
/*
  structure: 
  dragdropcontext which includes-> droppable -> all sections as draggable
  now each seaction has drapdropcontext -> droppable-> all chapters are considered draggable
