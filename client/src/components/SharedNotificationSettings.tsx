...
const SharedNotificationSettings = ({
  ...
  const { user } = useUser();
  const [updateUser] = useUpdateUserMutation(); // this is an arrya different. this is a function that we invoke.
  ...
  const onSubmit = async (data: NotificationSettingsFormData) => {
    ...
    const updatedUser = {
      ...
      publicMetadata: {
        ...user.publicMetadata,
        settings: {
          ...currentSettings, //first write the current settings.
          ...data, // then overwrite the settings with the data form the form
        },
      },
    };
